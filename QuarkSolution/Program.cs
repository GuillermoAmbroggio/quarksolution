    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while(opciones != 0)
            {
                ...
                switch (opciones)
                {
                    case 0:Console.WriteLine("Gracias por usar el programa, adios!");
                        break;
                    case 1:vendedor1.Cotizar(store1.camisas, store1.pantalones);
                        break;
                    case 2: Console.WriteLine("Estas son las cotizaciones echas hasta el momento por {0}:",vendedor1.Name);
                        vendedor1.Quotes.ForEach(e => Console.WriteLine($"Id:{e.Id}- Fecha: {e.Dates} -Codigo de Vendedor: {e.Code_seller} -Nombre Producto: {e.Name_clothing} {e.Type_clothing} -Cantidad: {e.Quantity_clothing} -Total: {e.Total}"));
                        break;
                    default: Console.WriteLine("Ingreso un valor fuera de rango");
                        break;
                }
